...
\author{Sebastien Chassot, Andre-Luc Robyr}
...
\begin{center}
%\begin{large}
\textbf{Sebastien Chassot, Andre-Luc Robyr} \\ Juin 2015
%\end{large}
\end{center}
...
ialloc(){
	...
	si not trouver:
		raise error aucun inode de libre
	
	inode trouver = occuper
	clear ancien inode
...
Si l'entrée était la dernière du dir on peut effacer ce block du dinode. Attention on pourrait créer 300 fichiers et effacer par hasard tous les fichiers du 3\up{e} block. Il faut donc supprimer le 3\up{e} mais décaler les suivants (la commande \emph{bmap()} renvoit les block qui se suivent)
